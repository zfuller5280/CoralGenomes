...
then
    ...
    #[ $(wc -l < "${vcf_file}") -lt 100 ] && print_help_and_exit 1 "VCF file (${vcf_file}) is too small to run this script!"
    ...
    then
        ...
        #[ $? != 0 ] && exit 1
        python ~/makeOutgroupPhasedFastas.py ~/Amil_v2.01/Amil.v2.01.chrs.fasta "${vcf_file}" > ${prefix_output}.fasta
    else
...
if [ ${seq_len} -lt 50 ]
then
    print_help_and_exit 1 "Error: The length of sequence is too short (< 50 bp) to construct a tree!\nPlease restart this script with different parameter values (-l, -m and/or -M)."
...
cp "${prefix_output}.phylip.txt" infile
if [ -z "${out_sample_id}" ]
then
    #echo -e "y\n" | "${DNAML}"; [ $? != 0 ] && exit 1
    echo "Is there an outgroup"
else
    out_sample_no=$[$(grep -ne "\<${out_sample_id}\>" infile | cut -f1 -d':') - 1]
    echo "Outgroup is ${out_sample_id} and ID number is ${out_sample_no}"
    ...
    then
        ...
        echo "Using outgroup ${out_sample_id}"
        echo "Running DNAML..."
        #sleep 2
        echo -e "o\n${out_sample_no}\ny\n" | "${DNAML}"; [ $? != 0 ] && exit 1
        #sleep 2
